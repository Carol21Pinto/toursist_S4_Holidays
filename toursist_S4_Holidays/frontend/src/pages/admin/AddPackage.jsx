...
const AddPackage = () => {
  ...
  // Itinerary
  const handleItineraryChange = (i, field, value) => {
    const next = [...itinerary]; next[i][field] = value; setItinerary(next);
  };
  ...
  // Inclusions / Exclusions (logic preserved)
  ...
  const triggerDashboardRefresh = () => {
    ...
    setTimeout(()=>localStorage.setItem('dashboardRefresh',(Date.now()+1).toString()),100);
    setTimeout(()=>localStorage.setItem('dashboardRefresh',(Date.now()+2).toString()),500);
  };
  ...
  const addPackage = async (packageData, cardImage) => {
    try {
      ...
      const response = await fetch(`${API_URL}/packages`, { method:"POST", body: formDataToSend });
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          setFormData({
            name:'', category:'Domestic', duration:'', pricePerPerson:'',
            currency:'INR', priceText:'', state:'', continent:'', groupType:''
          });
          setItinerary([{ day:1, title:'', activities:[''] }]);
          setInclusions(['']); setExclusions(['']); setDepartureDates(['']);
          setPricingNotes(['']); setPricingNoteCategories(['booking']);
          setCardImage(null); setImagePreview(null); setPricingMode('Structured');
        }, 1200);

        return newPackage;
      } else {
        const err = await response.json();
        throw new Error(err.message || `HTTP ${response.status}: Server Error`);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const packageData = {
      ...
      priceNote: pricingNotes.filter(n => n.trim()).join(' | '), // legacy text
      ...
      <form onSubmit={handleSubmit} className="add-package-form">
        ...
        <div className="form-section">
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input type="text" name="name" value={formData.name} onChange={handleInputChange} placeholder="India gate" required />
            </div>
                  ...
                  <div key={index} style={{
                    ...
                    <div style={{display:'flex', flexDirection:'column'}}>
                      <input
                        ...
                        placeholder={index===0? 'e.g., Booking Policy' : index===1? 'e.g., Full payment for flights required' : 'Add pricing note...'}
        ...
        <div className="form-section">
          ...
          <div className="form-group">
            ...
            <input type="file" accept="image/*" onChange={handleCardImageChange} id="cardImage" required />
            {imagePreview && (
